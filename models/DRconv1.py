...
class DRConv2d_v8_inception(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, region_num=8, **kwargs):
        ...
        self.conv_guide = nn.Conv2d(in_channels, region_num, kernel_size=kernel_size,padding=0, **kwargs)
    ...
    def forward(self, input):
        ...
        kernel_1 = kernel_1.view(kernel_1.size(0), -1, kernel_1.size(2), kernel_1.size(3))
        output_1 = self.corr(input, kernel_1, **self.kwargs)
        output_1 = output_1.view(output_1.size(0), self.region_num, -1, output_1.size(2), output_1.size(3))

        kernel_2 = self.conv_kernel_2(input)
        kernel_2 = kernel_2.view(kernel_2.size(0), -1, kernel_2.size(2), kernel_2.size(3))
        output_2 = self.corr(input, kernel_2, **self.kwargs)
        output_2 = output_2.view(output_2.size(0), self.region_num, -1, output_2.size(2), output_2.size(3))
