...
class cond_resample(nn.Module):
    def __init__(self,in_channel,out_channel,pyramid_channel=None):
        super(cond_resample,self).__init__()
        if pyramid_channel is not None:
            self.cond_resample = nn.Sequential(
                normalization(in_channel*3+pyramid_channel),
                SiLU(),
                nn.Conv2d(in_channel*3+pyramid_channel,out_channel,3,1,1)
            )
        else:
            self.cond_resample = nn.Sequential(
                normalization(in_channel*3),
                SiLU(),
                nn.Conv2d(in_channel*3,out_channel,3,1,1)
            )
        self.flip_sample = nn.Sequential(
            normalization(in_channel*2),
            SiLU(),
            nn.Conv2d(in_channel*2,in_channel,1,1)
            )
    def forward(self,diff_feature,cond,pyramid_x=None):
        cond_flip = torch.flip(cond,[2])
        condx = torch.cat((cond,cond_flip),1)
        cond_flip = self.flip_sample(condx)
 
        if pyramid_x is not None:
            mixed_feature = self.cond_resample(torch.cat((cond_flip,diff_feature,pyramid_x),1))
        else:
            mixed_feature = self.cond_resample(torch.cat((cond_flip,diff_feature),1))
        return mixed_feature,F.avg_pool2d(cond_flip,2)
...
class UNet(nn.Module):
    ...
    def __init__(
        ...
        self.cond_resample1 = cond_resample(64,128)
        self.cond_resample2 = cond_resample(128,256,64)
        self.cond_resample3 = cond_resample(256,512,128+64)

    def forward(self, x, gammas, cond):
        """
        ...
        for module in self.input_blocks:
            if i == 5: 
                h0 = h
                c = cond[0].detach()
                h, pyrimid_1 = self.cond_resample1(h,c)
                h = h+h0
            if i == 8:
                h0 = h
                c =cond[1].detach()
                h,pyrimid_2 = self.cond_resample2(h,c,pyrimid_1)
                h = h+h0
            if i == 11:
                h0 = h
                c = cond[2].detach()
                h,_ = self.cond_resample3(h,c,torch.cat([F.avg_pool2d(pyrimid_1,2),pyrimid_2],1))
                h = h+h0
            h = module(h, emb)
            hs.append(h)
            i=i+1
